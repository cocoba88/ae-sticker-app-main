...
-- ============================================
-- SMART FISHING STATE & TIMING SYSTEM
-- ============================================

local Smart = {
    Enabled = false,
    AutoTiming = true,          -- default selalu nyala di tab ini
    CatchSpeed = 6,             -- 1 (paling aman) - 10 (paling cepat)
    BiteDelays = {},
    CatchWindows = {},
    LastCast = 0,
    LastBite = 0,
    Samples = 0,
    MaxSamples = 25,
    AdjustedBiteDelay = 0.8,
    AdjustedCatchDelay = 0.12,
    IsRunning = false,
}

local function Smart_Calculate()
    if Smart.Samples < 8 then return end

    local function avg(t) local s=0 for _,v in ipairs(t) do s=s+v end return s/#t end
    local function stddev(t, mean) local s=0 for _,v in ipairs(t) do s=s+(v-mean)^2 end return math.sqrt(s/#t) end

    local biteAvg = avg(Smart.BiteDelays)
    local catchAvg = avg(Smart.CatchWindows)
    local biteStd = stddev(Smart.BiteDelays, biteAvg)
    local catchStd = stddev(Smart.CatchWindows, catchAvg)

    -- Speed factor: 1 = 0% reduction, 10 = max 60% reduction
    local reduction = (Smart.CatchSpeed - 1) / 9 * 0.60

    Smart.AdjustedBiteDelay  = biteAvg  * (1 - reduction * 0.3) + biteStd * 0.5
    Smart.AdjustedCatchDelay = catchAvg * (1 - reduction)       + catchStd * 0.6

    -- Safety floor
    Smart.AdjustedBiteDelay  = math.max(Smart.AdjustedBiteDelay,  0.35)
    Smart.AdjustedCatchDelay = math.max(Smart.AdjustedCatchDelay, 0.04)

    WindUI:Notify({
        Title = "Smart Timing Updated",
        Content = string.format("Bite %.3fs → Catch %.3fs\nSpeed Level: %d", 
...
-- ============================================
-- BITE DETECTION (via GUI + Remote fallback)
-- ============================================
...
task.spawn(function()
    ...
    while true do
        ...
        local gui = pgui:FindFirstChild("FishingMinigame") 
                 or pgui:FindFirstChild("Small Notification")
                 or pgui:FindFirstChild("Notification")

        if gui and not biteDetected then
            -- Cek tanda seru atau kata "Bite" / "!"
            local biteUI = gui:FindFirstChild("Exclamation") 
                        or gui:FindFirstChild("BiteIndicator")
                        or (gui:FindFirstChild("Display") and gui.Display:FindFirstChild("TextLabel") and gui.Display.TextLabel.Text:find("!"))

            if biteUI and (biteUI.Visible or biteUI.ImageTransparency < 1) then
                biteDetected = true
                Smart.LastBite = tick()
...
-- Hook fish caught (pasti catch sukses)
if _G.Remotes and _G.Remotes.FishCaught then
    _G.Remotes.FishCaught.OnClientEvent:Connect(function()
...
local function SmartFishingLoop()
    ...
    if _G.Remotes and _G.Remotes.EquipTool then
        pcall(function() _G.Remotes.EquipTool:FireServer(1) end)
        task.wait(0.8)
    end
    ...
    while Smart.Enabled do
        ...
        -- Charge + Start Minigame
        if _G.Remotes.ChargeRod then
            pcall(function() _G.Remotes.ChargeRod:InvokeServer(100) end)
            task.wait(0.06)
        end

        if _G.Remotes.StartMini then
            pcall(function() _G.Remotes.StartMini:InvokeServer(-1.233184814453125, 0.9945034885633273) end)
        end

        -- Tunggu bite (dengan adjusted delay sebagai fallback jika GUI gagal detect)
        local waited = 0
        while not biteDetected and waited < 4 do
            task.wait(0.03)
            waited = waited + 0.03
            if waited >= Smart.AdjustedBiteDelay then break end
        end

        -- Tarik ikan (spam sesuai adjusted catch delay)
        if biteDetected or waited >= Smart.AdjustedBiteDelay then
            for i = 1, 8 do
                if _G.Remotes.FinishFish then
                    pcall(function() _G.Remotes.FinishFish:FireServer() end)
                end
                task.wait(Smart.AdjustedCatchDelay / 7)
            end
        end

        task.wait(0.12) -- cooldown antar cast
    end
...
local smartToggle = SmartFishingTab:Toggle({
    Title = "Enable Smart Fishing",
    Desc = "Full auto-detection + auto-adjust timing",
    Value = false,
    Callback = function(v)
        Smart.Enabled = v
        if v then
            ...
            Smart.Samples = 0
            WindUI:Notify({
                Title = "Smart Fishing Active",
                Content = "Sedang mengumpulkan data timing...",
                Duration = 6,
                Icon = "bot"
            })
            task.spawn(SmartFishingLoop)
        else
            WindUI:Notify({Title="Smart Fishing Stopped", Content="Metode lain bisa dipakai kembali", Duration=4, Icon="bot"})
        end
    end
})

myConfig:Register("SmartFishing_Enabled", smartToggle)

SmartFishingTab:Space()

SmartFishingTab:Slider({
    Title = "Catch Speed Level",
    Desc = "1 = Sangat aman • 10 = Ekstrem cepat",
...
SmartFishingTab:Button({
    Title = "Reset Timing Data",
    Desc = "Hapus semua data kalibrasi & mulai dari awal",
    Callback = function()
        Smart.BiteDelays = {}
        Smart.CatchWindows = {}
        Smart.Samples = 0
        Smart.AdjustedBiteDelay = 0.8
        Smart.AdjustedCatchDelay = 0.12
        WindUI:Notify({Title="Data Reset", Content="Timing data telah direset", Duration=3, Icon="rotate-cw"})
    end
})

SmartFishingTab:Space()

SmartFishingTab:Paragraph({
    Title = "Real-time Info",
    Desc = function()
        return string.format(
            "Samples: %d\nBite Delay: %.3fs\nCatch Window: %.3fs\nSpeed Level: %d",
            Smart.Samples,
            Smart.AdjustedBiteDelay,
            Smart.AdjustedCatchDelay,
            Smart.CatchSpeed
        )
    end,
    RefreshRate = 1
...
print("Smart Fishing Tab Loaded! (Auto Timing Detection)")

-- Selesai! Tab ini 100% independen dari X5 Speed & Auto Fishing New Method
...
local Window = WindUI:CreateWindow({
    Title = "TestTeam Hub | Only Test",
    Icon = "crown",
    Author = "TestTeam | Only Test",
    Folder = "TestTeam",
...
local SmartFishingTab = AllMenu:Tab({
    Title = "Smart Fishing",
    Icon = "bot"
})
...
SmartFishingTab:Section({
    Title = "Smart Auto-Fishing (Auto Timing Detection)",
    TextSize = 22,
    TextXAlignment = "Center",
    Opened = true
})
